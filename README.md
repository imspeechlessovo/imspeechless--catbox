...
```sql
# SQLite 无需手动创建数据库，首次运行自动生成 dev.db 文件
```
...
```env
DATABASE_URL="SQLite://postgres:你的密码@localhost:5432/maoxiang_countdown"
...
**Q: 种子数据跑完后如何重置？**
A: `删除 dev.db 文件后重新运行 npm run seed` 会清空数据库并重新运行迁移和种子。
