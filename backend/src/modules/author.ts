...
authorRouter.post('/messages', requireAuthor, (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!t || t.length > 100) return res.status(400).json({ error: '标题需要在1-100个字符之间' });
    if (!c || c.length > 500) return res.status(400).json({ error: '内容需要在1-500个字符之间' });
...
authorRouter.patch('/messages/:id', requireAuthor, (req: AuthRequest, res: Response) => {
  try {
    ...
    if (body.title !== undefined) { const t = String(body.title).trim(); if (!t || t.length > 100) return res.status(400).json({ error: '标题需要在1-100个字符之间' }); updates.title = t; }
    if (body.content !== undefined) { const c = String(body.content).trim(); if (!c || c.length > 500) return res.status(400).json({ error: '内容需要在1-500个字符之间' }); updates.content = c; }
    ...
    if (!updated) return res.status(404).json({ error: '留言不存在' });
...
authorRouter.post('/questions', requireAuthor, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!t || t.length > 200) return res.status(400).json({ error: '问题内容需要在1-200个字符之间' });
    if (!a) return res.status(400).json({ error: '请输入答案' });
...
authorRouter.patch('/questions/:id', requireAuthor, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const body = req.body as Record<string, unknown>;
    const updates: Record<string, unknown> = { updated_at: new Date().toISOString() };
    if (body.title !== undefined) { const t = String(body.title).trim(); if (!t || t.length > 200) return res.status(400).json({ error: '问题内容需要在1-200个字符之间' }); updates.title = t; }
    if (body.answer !== undefined) { const a = String(body.answer).trim(); if (!a) return res.status(400).json({ error: '请输入答案' }); updates.answer_hash = await bcrypt.hash(normalizeAnswer(a), 10); }
